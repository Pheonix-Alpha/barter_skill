...
public class LessonService {
    ...
    /**
     * Schedule a new lesson request, only if an accepted exchange request exists.
     */
  public Lesson scheduleLesson(Long senderId, Long receiverId, Long skillId, LocalDateTime time, int duration, String notes) {
    ...
    boolean hasAccepted = requestRepo.existsByParticipantsAndSkillAndAcceptedStatus(sender.getId(), receiver.getId(), skillId);
    if (!hasAccepted) {
        throw new IllegalStateException("You must have an accepted exchange request before scheduling a lesson.");
    }

   if (lessonRepo.existsBySenderIdAndReceiverIdAndSkillIdAndScheduledTime(
    sender.getId(), receiver.getId(), skillId, time)) {
    ...
    lesson.setPlatformLink(null); // To be added later
